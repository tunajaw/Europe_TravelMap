...
describe('FR-MAP-03/04 Country Map', () => {
  ...
  it('shows the country name and all its reviewed cities, with hover and keyboard emphasis', async () => {
    ...
    const berlin = within(layer).getByRole('button', { name: 'Berlin' });
    ...
    expect(within(layer).queryByRole('button', { name: 'Vatican City' })).not.toBeInTheDocument();
  });
  ...
  it('returns to Europe and removes city markers without losing the photo-selection workflow', async () => {
    ...
    expect(within(screen.getByRole('group', { name: 'Visited Cities' })).getByRole('button', { name: 'Vatican City' })).toBeInTheDocument();
  });
});
