...
。無論真實出發的城市為何，在 Europe Map 中一律以地圖上的顯示點為端點；在 Country Map 中以城市上的顯示點為端點；在 City Map 中不顯示 Segment。 紅色: 高鐵、淺綠色: 客運、淺藍色: 渡輪/郵輪、深藍色: 飛機、粉色: 火車
...
FR-MAP-09 (Segment Path metadata): 包含出發點-抵達點，日期，價錢，交通方式，備註。
...
Transportation :
畫面左上方有四種交通類別的按鈕(類別 div 本體是按鈕):
Plane: Total 550/Avg 20
Bus: Total 120/Avg 17
Train: ...
Ferry: ...
...
* Segment 距離為起點城市 Hbf 到終點城市 Hbf 的直線距離。 checkbox "考慮市區到機場接駁"，決定 Airport Transfer 算不算在 Transportation 統計裡。如果需要計算，且該單程機場接駁為非0元，要把若啟用「考慮市區到機場接駁」，且對應 Airport Transfer 的單程費用非 0 元，則將該費用加入對應 Segment 的分析費用；同時將該 Airport Transfer 的 Hbf → Airport 直線距離加入 Segment 的分析距離。
...
橫向 barplot，列出所有篩選過後的 Segment，如果數值相同以時間排序前面者優先。每個 bar 最右邊顯示數值，Bar 顯示合併機場接駁 (如有勾選 checkbox) 的費用。bar 的顏色跟 Map Segment 一樣，紅色: 高鐵、淺綠色: 客運、淺藍色: 渡輪/郵輪、深藍色: 飛機、粉色: 火車。 
...
FR-EXR-11 (Accommodation Filter)
...
橫向 barplot，列出所有篩選過後的 Accommodation，如果數值相同以時間排序前面者優先。每個 bar 最右邊顯示數值。一個 Accommodation = 一個 bar，bar 的顏色跟 Map Segment 一樣，粉色: Airbnb，淺藍色: Hotel，如果勾選"考慮機場過夜" (黃色但因為0元不會顯示): Airport。
